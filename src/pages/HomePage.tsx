...
export default function HomePage() {
  ...
  const pageResetKey = `${searchKeyword}|${selectedCategoryIds.join(",")}|${selectedPriceRangeIds.join(",")}|${sort}`;
  ...
  const setPage = (nextPage: number | ((current: number) => number)) => {
    setPageState((current) => {
      const currentPage = current.key === pageResetKey ? current.page : 1;
      const pageValue =
        typeof nextPage === "function" ? nextPage(currentPage) : nextPage;
      return { key: pageResetKey, page: pageValue };
    });
  };
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products
      ...
      .filter((product) =>
        isProductInPriceRanges(product, selectedPriceRangeIds),
      )
      .sort((a, b) => {
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="border-r border-gray-200 py-4 pr-5">
          <section className="border-b border-gray-200 pb-6">
            <h2 className="mb-4 text-xl font-medium uppercase text-red-600">
              Loại sản phẩm
            </h2>
            <div className="space-y-3">
              {visibleCategories.map((category) => (
                <label
                  key={category.id}
                  className="flex cursor-pointer items-start gap-3 text-lg leading-snug text-gray-950"
                >
                  <input
                    type="checkbox"
          ...
          <section className="pt-5">
            ...
            <div className="space-y-3">
              {priceRanges.map((range) => (
                <label
                  key={range.id}
                  className="flex cursor-pointer items-center gap-3 text-lg text-gray-950"
                >
                  <input
                    type="checkbox"
        ...
        <section className="min-w-0 py-3">
          <h1 className="mb-6 text-3xl font-bold uppercase text-gray-700">
            SHOP BÌNH - DỤNG CỤ HỌC TẬP
          </h1>
          ...
          {isLoading ? (
            ...
            <div className="py-16 text-center text-red-600">
              Không thể tải sản phẩm. Vui lòng kiểm tra backend và cấu hình
              CORS.
            </div>
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
                {paginatedProducts.map((product) => (
                  <Link
                    ...
                    className="group relative flex min-h-[560px] flex-col overflow-hidden rounded-[8px] border border-gray-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-red-500"
                  >
                    {product.image_url ? (
                      <img
                    ...
                    {product.stock <= 0 && (
                      <span className="absolute left-3 top-3 rounded-full bg-gray-900 px-3 py-1 text-xs font-bold uppercase text-white">
                        Hết hàng
                      </span>
                    )}
                    ...
                    <div className="flex flex-1 flex-col px-4 py-4">
                      ...
                      <p className="mt-2 text-center text-2xl font-bold text-red-600">
                        {formatPrice(product.price)}
                      </p>
                      <button
                        type="button"
                        className={`mt-auto rounded-full border px-5 py-2 text-sm font-bold uppercase transition-colors ${
                          product.stock <= 0
                            ? "border-gray-300 text-gray-400"
                            : "border-red-600 text-red-600 group-hover:bg-red-600 group-hover:text-white"
                        }`}
                      >
                        {product.stock <= 0 ? "Hết hàng" : "Xem nhanh"}
                      </button>
                    </div>
                  </Link>
                ))}
