...
export default function ProductDetailPage() {
  ...
  if (!Number.isFinite(productId) || productId <= 0 || isError) {
    return (
      <div className="mx-auto max-w-[1700px] bg-white px-5 py-16 text-center lg:px-8">
        <h1 className="mb-4 text-2xl font-bold text-gray-900">Không tìm thấy sản phẩm</h1>
  ...
  return (
    <div className="mx-auto max-w-[1700px] bg-white px-5 py-8 lg:px-8">
      <Link to="/" className="mb-5 inline-flex items-center font-semibold text-red-600 hover:text-red-700">
        ← Quay lại sản phẩm
      </Link>
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,0.95fr)_minmax(420px,0.8fr)]">
        ...
        <div className="rounded-[8px] border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <h1 className="text-3xl font-bold leading-tight text-gray-950">{product.name}</h1>
          ...
          <p className="mt-5 text-4xl font-bold text-red-600">{formatPrice(product.price)}</p>

          <div className="mt-6 rounded-[8px] bg-gray-50 p-5">
            <h2 className="mb-3 text-lg font-bold text-gray-900">Mô tả sản phẩm</h2>
          ...
          <div className="mt-6 border-t border-gray-200 pt-6">
            <label className="mb-3 block text-lg font-bold text-gray-900">Số lượng</label>
            <div className="mb-6 flex h-12 w-fit items-center rounded-full border border-gray-300">
              <button
                type="button"
                onClick={() => setQuantity((value) => Math.max(1, value - 1))}
                className="h-12 w-12 rounded-l-full text-xl font-bold text-gray-700 transition-colors hover:bg-red-50 hover:text-red-600"
              >
                -
              </button>
              <input
              ...
              <button
                ...
                className="h-12 w-12 rounded-r-full text-xl font-bold text-gray-700 transition-colors hover:bg-red-50 hover:text-red-600"
              >
                +
              </button>
            </div>
